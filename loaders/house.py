import math
import numpy as np
from loaders.svg_utils import PolygonWall, get_polygon, calc_distance, get_room_number, get_icon, get_icon_number, get_points, get_direction, get_gaussian2D
...
class House:
    def __init__(self, path, height, width, icon_list=icons_selected, room_list=rooms_selected):
        ...
        self.walls = np.empty((height, width), dtype=np.int32)
        self.walls.fill(0)
        self.wall_ids = np.empty((height, width), dtype=np.int32)
        self.wall_ids.fill(0)
        self.icons = np.zeros((height, width), dtype=np.int32)
        # junction_id = 0
        wall_id = 1
        self.wall_ends = []
        self.wall_objs = []
        ...
        for e in svg.getElementsByTagName('g'):
            ...
            if e.getAttribute("id") == "Window":
                ...
                self.icons[cc, rr] = icon_list["Window"]
            ...
            if e.getAttribute("id") == "Door":
                ...
                self.icons[cc, rr] = icon_list["Door"]
